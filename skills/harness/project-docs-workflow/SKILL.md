...
3. 按真值类型核对上下文：合同管 expected behavior，代码管 implemented behavior，artifact 管 evidence
4. 识别 `requirements.md` 与 current implementation 的差异
5. 实施前冻结模块拆分、依赖方向、公共接口、复用边界、文件行数预算和测试归属
...
- 把 owning `requirements.md` / frozen contract 视为 expected behavior 真值
...
- 外部 proposal 只作为 recommendation/provenance；只有 adoption matrix 中 `ACCEPTED` 或
  `MODIFIED` 且已转译到内部 requirements/design/development plan 的条目可以进入实施
...
非 trivial 实施前，在 owning README 或 development plan 持久化：
...
没有此模块设计时，不得直接从一个新大文件开始堆叠功能。已有或预计超过
...
- 当前代码决定“现在实现了什么”
- owning requirements / frozen contract 决定“应该实现什么”
...
- `docs/feature/<module>/changelog.md`（仅 milestone / gate 级 append-only 记录）
