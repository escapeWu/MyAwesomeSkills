...
- active Spec（任务涉及 schema、state、module、migration、security 或 validation contract 时必须读取）
- active Spec 引用的 blocking ADR / system decision（只读相关 decision，不扫描全部历史）
...
- frozen Spec 与 requirements/accepted ADR 冲突时，记录 contract/decision gap 并停止把该 Spec当作 implementation-ready
- `proposed` ADR 只是候选决策，不得作为已接受架构事实
...
- 如由 harness 调用，生成可直接用于实施的 `Implementation Context` 摘要：requirements、active Spec、accepted ADR、入口点、相关文件、风险、验证建议
- 识别满足 durable decision trigger 的 ADR candidate；只报告候选及证据，不把未确认决策写进 README
...
写入前至少检查：
- 目标文档是否选对（requirements / active Spec / ADR / Feature README / reference / OVERVIEW）
- 若为更新，是否保留了仍然正确的旧内容
...
2. active `docs/feature/<module>/specs/SPEC-*.md`（bounded implementation contract）
3. owning Feature/system ADR（仅满足 durable decision trigger 时）
4. `docs/feature/<category>/README.md`
5. `docs/feature/<category>/<module>/README.md` 或 `INDEX.md`
6. `docs/reference/*.md`
7. `docs/OVERVIEW.md`

优先规则：
- expected behavior、验收口径、产品 / 业务行为，优先回填 owning feature 的 `requirements.md`
- bounded implementation design、schema/state、模块 ownership、migration 与 validation matrix，优先回填 active Spec
- 满足 trigger 的 durable architecture rationale 回填 ADR；可逆局部设计留在 Spec
- current implementation、API/status、模块实现、变更记录，优先回填 `docs/feature/<category>/...` 的 README/INDEX
- stable cross-Feature interface contracts、外部系统、第三方 API、架构参考、限制条件，优先回填 `docs/reference/interfaces.md` 或其他相关 reference 文件
...
- owning GOAL / README / requirements / active Spec / ADR 候选路径
- 需要补足的缺口：架构边界、数据流、时序、性能风险、contract 命名或验证证据
- 期望产出是否要被后续实施直接引用

收到这些信息时，本技能应：
- 优先服务 planning gate：补足实施所需的代码事实、文档落点、风险边界和验证建议
- 输出一个 **Implementation Context** section，包含 `Expected Behavior Source`、`Active Spec`、`Accepted Decisions`、`Entry Points`、`Relevant Files`、`Contracts / Data Shapes`、`Risks / Open Questions`、`Validation Candidates`；expected behavior 优先引用 `requirements.md`
