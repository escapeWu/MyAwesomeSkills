...
description: 项目 docs 维护编排器。用于非 trivial 的开发任务、功能开发、bug 修复、重构、API 变更、跨模块修改前后：先扫描 docs/OVERVIEW.md、feature-*、reference-*，把相关文档作为半可信上下文；判断是否需要升级使用 project-analysis 做深度分析；实现完成后判断是否应 patch feature/reference/OVERVIEW/历史教训，并先询问用户确认后再更新。看到用户要“实现/修改/开发/修复”项目代码时，应优先触发此技能，而不是等用户主动提及 docs。
---
...
- 明确告诉它最终知识应优先回填 `feature-*` / `reference-*` / `OVERVIEW.md`
- 不要默认生成平行的长期文档体系
- 除非用户明确要独立分析报告
...
- 优先把这些 section 合并进现有 `feature-*` / `reference-*`
- 只有用户明确要求独立分析稿时，才保留独立分析文档
