...
| [harness/code-organization-harness](./skills/harness/code-organization-harness) | Decide where code/tests/docs live: discover context via targeted grep, choose module boundaries, place files by domain and layer (Next.js / Python / Java patterns). |
| [harness/codex-design-review](./skills/harness/codex-design-review) | Targeted design/code review via Codex CLI; outputs a structured review report under `docs/feature/<module>/review-<slug>.md`. |
| [harness/harness-engineering-plan](./skills/harness/harness-engineering-plan) | Decompose goals into milestones / TaskNodes / waves / gates. Mandatory taskBoard + parallel multi-agent dispatch (Cursor `Task` tool or `codex` CLI) before any non-trivial multi-step implementation. |
